...
function Write-Log {
	...
	#region Parameters
		...
		Param(
			...
			[Parameter()]
			[IO.FileInfo] $Path = ".\..\Nuget\NuGet.log",
...
Write-Log " "
Write-Log "Creating package $PackageName" -ForegroundColor Green

# Create symbols package if any .pdb files are located in the lib folder
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process .\..\Nuget\NuGet.exe ("pack " + $PackageName + ".csproj -Symbol -Verbosity Detailed -OutputDirectory ..\Nuget\")
...
Else {
	$packageTask = Create-Process .\..\Nuget\NuGet.exe ("pack  " + $PackageName + ".csproj -Verbosity Detailed -OutputDirectory ..\Nuget\")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit()
